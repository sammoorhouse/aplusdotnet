...
namespace AplusCore.Runtime.Function.Dyadic.Scalar
{
    abstract class DyadicScalar : AbstractDyadicFunction
    {
        ...
        /// <summary>
        /// Stores the allowed methods. The key is a combination generated by <see cref="ATypeMask"/>,
        ...
        {
            byte typeMask = ATypeMask(right.Type, left.Type);

            if (right.Type == ATypes.ANull || left.Type == ATypes.ANull)
            {
                typeMask = ATypeMask(ATypes.ANull, ATypes.ANull);
            }
            // Check if we have a rule for the specific input types
            else if (!this.allowedMethods.ContainsKey(typeMask))
            {
                // Rule not found
                // reset the combination info to the default case
                typeMask = ATypeMask(ATypes.AType, ATypes.AType);
                // throw an error if we don't have a default case
                if (!this.allowedMethods.ContainsKey(typeMask))
                {
                    // throw a type error
            ...
            return ExecuteRecursion(right, left, typeMask);
        }
        ...
        private AType ExecuteRecursion(AType rightArgument, AType leftArgument, byte typeMask)
        {
            AType result;
            ...
            if (leftArgument.IsArray && rightArgument.IsArray)
            {
                ...
                {
                    currentItem = ExecuteRecursion(rightArgument[i], leftArgument[i], typeMask);
            ...
            else if (leftArgument.IsArray && !rightArgument.IsArray)
            {
                ...
                for (int i = 0; i < leftArgument.Length; i++)
                {
                    currentItem = ExecuteRecursion(rightArgument, leftArgument[i], typeMask);
                    floatTypeCounter += (uint)((currentItem.Type == ATypes.AFloat) ? 1 : 0);
            ...
            else if (!leftArgument.IsArray && rightArgument.IsArray)
            {
                ...
                {
                    currentItem = ExecuteRecursion(rightArgument[i], leftArgument, typeMask);
                    floatTypeCounter += (uint)((currentItem.Type == ATypes.AFloat) ? 1 : 0);
            ...
            else if (typeMask == ATypeMask(ATypes.ANull, ATypes.ANull))
            {
                return Utils.ANull();
            }
            else
            {
                ...
                MethodInfo method = this.allowedMethods[typeMask];
